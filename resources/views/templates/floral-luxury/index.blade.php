...
<head>
    ...
    <style>
        ...
        /* ── Global falling petal canvas (main content) ─────────── */
        #floral-petals { position:fixed;inset:0;pointer-events:none;z-index:92;overflow:hidden; }
        .fl-petal      { position:absolute; }
        @keyframes flPetalFall {
            0%   { transform:translateY(-80px) translateX(0) rotate(0deg); opacity:0; }
            8%   { opacity:1; }
            90%  { opacity:0.75; }
            100% { transform:translateY(110vh) translateX(var(--drift,50px)) rotate(var(--spin,480deg)); opacity:0; }
        }

        /* ── Cover petal canvas (inside cover div) ───────────────── */
        #fl-cover-petals { position:absolute;inset:0;pointer-events:none;overflow:hidden;z-index:0; }
        /* ── Cover static rose ornament float ── */
        .fl-rose-deco {
            position:absolute;pointer-events:none;
            animation:flRoseFloat 6s ease-in-out infinite;
        }
        @keyframes flRoseFloat {
        ...
        .fl-in   { animation:flSlideUp 0.9s ease-out both; }
        .fl-d1   { animation-delay:0.2s; }
        .fl-d2   { animation-delay:0.5s; }
        .fl-d3   { animation-delay:0.8s; }
...
<div data-coi-cover
    ...
    {{-- Cover petal particle canvas --}}
    <div id="fl-cover-petals"></div>

    {{-- Decorative elements --}}
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute -top-20 -left-20 w-80 h-80 rounded-full" style="background: radial-gradient(circle, rgba(48,109,41,0.06) 0%, transparent 70%)"></div>
        <div class="absolute -bottom-20 -right-20 w-80 h-80 rounded-full" style="background: radial-gradient(circle, rgba(48,109,41,0.08) 0%, transparent 70%)"></div>

        {{-- Floating rose ornaments (corners) --}}
        <svg class="fl-rose-deco w-20 h-20 opacity-15" style="top:4%;left:2%;animation-delay:0s" viewBox="0 0 80 80" fill="none">
            <circle cx="40" cy="40" r="18" fill="#f9a8d4" opacity=".5"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fbcfe8" transform="rotate(0,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fbcfe8" transform="rotate(45,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fbcfe8" transform="rotate(90,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fbcfe8" transform="rotate(135,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fce7f3" transform="rotate(180,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fce7f3" transform="rotate(225,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fce7f3" transform="rotate(270,40,40)"/>
            <ellipse cx="40" cy="25" rx="8" ry="13" fill="#fce7f3" transform="rotate(315,40,40)"/>
            <circle cx="40" cy="40" r="5" fill="#f472b6" opacity=".6"/>
        </svg>
        <svg class="fl-rose-deco w-14 h-14 opacity-20" style="top:6%;right:4%;animation-delay:-2s" viewBox="0 0 80 80" fill="none">
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#f9a8d4" transform="rotate(0,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#f9a8d4" transform="rotate(60,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fbcfe8" transform="rotate(120,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fbcfe8" transform="rotate(180,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fce7f3" transform="rotate(240,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fce7f3" transform="rotate(300,40,40)"/>
            <circle cx="40" cy="40" r="5" fill="#f472b6" opacity=".5"/>
        </svg>
        <svg class="fl-rose-deco w-16 h-16 opacity-15" style="bottom:8%;left:3%;animation-delay:-3.5s" viewBox="0 0 80 80" fill="none">
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fce7f3" transform="rotate(0,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fbcfe8" transform="rotate(72,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#f9a8d4" transform="rotate(144,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fbcfe8" transform="rotate(216,40,40)"/>
            <ellipse cx="40" cy="26" rx="7" ry="12" fill="#fce7f3" transform="rotate(288,40,40)"/>
            <circle cx="40" cy="40" r="4" fill="#f472b6" opacity=".5"/>
        </svg>
        <svg class="fl-rose-deco w-12 h-12 opacity-20" style="bottom:10%;right:5%;animation-delay:-1.5s" viewBox="0 0 80 80" fill="none">
            <ellipse cx="40" cy="27" rx="6" ry="11" fill="#f9a8d4" transform="rotate(0,40,40)"/>
            <ellipse cx="40" cy="27" rx="6" ry="11" fill="#fbcfe8" transform="rotate(90,40,40)"/>
            <ellipse cx="40" cy="27" rx="6" ry="11" fill="#fce7f3" transform="rotate(180,40,40)"/>
            <ellipse cx="40" cy="27" rx="6" ry="11" fill="#f9a8d4" transform="rotate(270,40,40)"/>
            <circle cx="40" cy="40" r="4" fill="#ec4899" opacity=".4"/>
        </svg>
    </div>

    <div class="text-center px-8 max-w-sm relative z-10">
        <div class="mb-8">
            <p class="fl-in fl-d1 font-cormorant text-forest/60 text-sm tracking-[0.3em] uppercase mb-4">The Wedding of</p>
            <h1 class="fl-in fl-d2 font-serif-luxury text-5xl text-forest leading-tight mb-2">
                {{ $invitation->groom_name }}
            </h1>
            <p class="fl-in fl-d2 font-cormorant text-2xl text-forest/50 italic my-2">&</p>
            <h1 class="fl-in fl-d3 font-serif-luxury text-5xl text-forest leading-tight">
                {{ $invitation->bride_name }}
            </h1>
        </div>

        @if($guest)
        <div class="fl-in fl-d3 mb-8 p-4 border border-forest/20 rounded-2xl bg-white/40">
        ...
        <button data-coi-btn @click="opened = true; $store.invitation.openEnvelope()"
                class="fl-in fl-d4 group relative inline-flex items-center gap-3 px-8 py-4 rounded-2xl text-cream font-medium text-sm tracking-wide shadow-lg hover:shadow-xl transition-all duration-300 active:scale-95"
        ...
        <p class="fl-in fl-d5 text-xs text-forest/40 mt-6 font-cormorant italic">Sentuh untuk membuka</p>
    </div>
</div>
...
<div data-coi-main
    ...
    <section id="nav-top" class="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div class="absolute inset-0 overflow-hidden">
            <img src="{{ $invitation->cover_photo_url }}" alt="Cover"
                 class="w-full object-cover" style="height:130%;top:-15%;position:absolute;left:0;right:0"
                 data-parallax-hero>
    ...
    <section id="nav-gallery" class="py-16" style="background:rgba(255,255,255,0.4)">
                ...
                <div class="card-luxury p-5" data-aos="fade-up">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            @endif
                            @if($gift->qris_image)
                            <img src="{{ Storage::url($gift->qris_image) }}" alt="QRIS" class="mt-3 max-w-[200px] rounded-xl shadow-sm">
                            @endif
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // ── Falling Flower Petals (cover + global canvas) ────────────
    (function () {
        var COLORS = [
            ['#fce7f3','#fbcfe8'], // blush pink
            ['#fbcfe8','#f9a8d4'], // rose pink
            ['#f9a8d4','#f472b6'], // deeper pink
            ['#fef3c7','#fde68a'], // cream yellow
            ['#dcfce7','#bbf7d0'], // light mint
            ['#ffffff','#f3f4f6'], // white
        ];

        function mkSvg(c1, c2, t) {
            if (t === 0) {
                return '<svg viewBox="0 0 20 34" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">'
                     + '<path d="M10 1C16 9,17 22,10 33C3 22,4 9,10 1Z" fill="'+c1+'"/>'
                     + '<path d="M10 5C13 12,14 22,10 29C7 22,7 12,10 5Z" fill="'+c2+'" opacity=".45"/>'
                     + '</svg>';
            }
            if (t === 1) {
                return '<svg viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">'
                     + '<g transform="translate(25,25)">'
                     + '<ellipse rx="6" ry="13" transform="rotate(0)"   fill="'+c1+'"/>'
                     + '<ellipse rx="6" ry="13" transform="rotate(72)"  fill="'+c1+'"/>'
                     + '<ellipse rx="6" ry="13" transform="rotate(144)" fill="'+c1+'"/>'
                     + '<ellipse rx="6" ry="13" transform="rotate(216)" fill="'+c1+'"/>'
                     + '<ellipse rx="6" ry="13" transform="rotate(288)" fill="'+c1+'"/>'
                     + '<circle r="4" fill="'+c2+'"/>'
                     + '</g></svg>';
            }
            return '<svg viewBox="0 0 30 22" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">'
                 + '<ellipse cx="15" cy="11" rx="14" ry="8" fill="'+c1+'" transform="rotate(-25 15 11)"/>'
                 + '<ellipse cx="15" cy="11" rx="9"  ry="5" fill="'+c2+'" opacity=".5" transform="rotate(-25 15 11)"/>'
                 + '</svg>';
        }

        function spawnPetals(canvasId, N) {
            var c = document.getElementById(canvasId);
            if (!c) return;
            for (var i = 0; i < N; i++) {
                var cp    = COLORS[Math.floor(Math.random() * COLORS.length)];
                var t     = Math.floor(Math.random() * 3);
                var sz    = t === 1 ? (10 + Math.random() * 14) : (8 + Math.random() * 18);
                var dur   = 9 + Math.random() * 14;
                var drift = (Math.random() > .5 ? 1 : -1) * (30 + Math.random() * 90);
                var spin  = (Math.random() > .5 ? 1 : -1) * (300 + Math.random() * 440);
                var el    = document.createElement('div');
                el.className = 'fl-petal';
                el.style.cssText =
                    'left:'   + (Math.random() * 100) + '%;'
                  + 'top:-80px;'
                  + 'width:'  + sz + 'px;'
                  + 'height:' + (t === 1 ? sz : sz * 1.5) + 'px;'
                  + '--drift:' + drift + 'px;'
                  + '--spin:'  + spin  + 'deg;'
                  + 'animation:flPetalFall ' + dur + 's linear ' + (-(Math.random() * dur)) + 's infinite;';
                el.innerHTML = mkSvg(cp[0], cp[1], t);
                c.appendChild(el);
            }
        }

        spawnPetals('fl-cover-petals', 22); // cover screen (absolute inside cover div)
        spawnPetals('floral-petals',   26); // main content overlay (fixed z-92)
    })();
</script>
</body>
</html>
